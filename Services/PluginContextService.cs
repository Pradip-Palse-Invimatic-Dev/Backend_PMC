...
namespace MyWebApp.Api.Services
{
    ...
    public class PluginContextService : IPluginContextService
    {
        ...
        {
            if (!Guid.TryParse(entityId, out var applicationId))
            {
            ...
            return new
            {
                ...
                Price = "3000" // Fixed price for now, can be made configurable
            };
        }
        ...
        private async Task<dynamic> HandleBillDeskService(dynamic input)
        {
            try
            {
                // For demo purposes, we'll mock the encryption/decryption
                // In production, this would call the actual BillDesk encryption service

                string action = input.Action;

                if (action == "Encrypt")
                {
                    // Mock encryption - return a sample encrypted payload
        ...
        private async Task<dynamic> HandleChallanService(dynamic input)
        {
            try
            {
                ...
                // Generate PDF using QuestPDF (similar to the original plugin)
        ...
        {
            // Implementation using QuestPDF similar to the original plugin
        ...
        private void ComposeChallanContent(QuestPDF.Fluent.TextDescriptor text, dynamic model)
        {
            try
            {
                ...
                // Use only available Nirmala UI for Marathi text
                text.Line("               चलन पावती")
                ...
                text.Line($" {model.Date}").FontFamily("Arial").LineHeight(1);
            }
            catch (Exception)
            {
